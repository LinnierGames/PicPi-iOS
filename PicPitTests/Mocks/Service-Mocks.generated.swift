...
// MARK: - AsyncProviderProtocol
class AsyncProviderProtocolMock: AsyncProviderProtocol {
  ...
  private var expectationForGet: XCTestExpectation?
  private var expectGetBlock: () throws -> Promise<Value> = {  
    XCTFail("unexpected call to AsyncProviderProtocol.get")
    fatalError()
  }

  @discardableResult
  func expectGet(_ block: @escaping () throws -> Promise<Value>) -> XCTestExpectation {
    self.expectationForGet = self.testCase.expectation(description: "AsyncProviderProtocol.get")
    self.expectGetBlock = block
    return self.expectationForGet!
  }

  func get() -> Promise<Value> {
    XCTAssertNotNil(self.expectationForGet)
    self.expectationForGet?.fulfill()
    do {
      return try self.expectGetBlock()
    } catch {
  ...
  private var expectationForEreased: XCTestExpectation?
  private var expectEreasedBlock: () throws -> AnyAsyncProvider<Value> = {  
    XCTFail("unexpected call to AsyncProviderProtocol.ereased")
    fatalError()
  }

  @discardableResult
  func expectEreased(_ block: @escaping () throws -> AnyAsyncProvider<Value>) -> XCTestExpectation {
    self.expectationForEreased = self.testCase.expectation(description: "AsyncProviderProtocol.ereased")
    self.expectEreasedBlock = block
    return self.expectationForEreased!
  }

  func ereased() -> AnyAsyncProvider<Value> {
    XCTAssertNotNil(self.expectationForEreased)
    self.expectationForEreased?.fulfill()
    do {
      return try self.expectEreasedBlock()
    } catch {
      XCTFail("unexpecting thrown error: \(error.localizedDescription)")
      fatalError()
    }
...
class PhotoUploadManagerDelegateMock: PhotoUploadManagerDelegate {
  ...
  // MARK: manager(_ manager: PhotoUploadManager,
               didFinishPickingPhotos finished: Bool  )

  private var expectationForManagerManagerFinished: XCTestExpectation?
  private var expectManagerManagerFinishedBlock: (PhotoUploadManager, Bool) throws -> Void = { _,_  in
    XCTFail("unexpected call to PhotoUploadManagerDelegate.manager(_:didFinishPickingPhotos:)")
    fatalError()
  }

  @discardableResult
  func expectManagerManagerFinished(_ block: @escaping (PhotoUploadManager, Bool) throws -> Void) -> XCTestExpectation {
    self.expectationForManagerManagerFinished = self.testCase.expectation(description: "PhotoUploadManagerDelegate.manager(_:didFinishPickingPhotos:)")
    self.expectManagerManagerFinishedBlock = block
    return self.expectationForManagerManagerFinished!
  }

  func manager(_ manager: PhotoUploadManager,
               didFinishPickingPhotos finished: Bool  ) -> Void {
    XCTAssertNotNil(self.expectationForManagerManagerFinished)
    self.expectationForManagerManagerFinished?.fulfill()
    do {
      return try self.expectManagerManagerFinishedBlock(manager, finished)
    } catch {
      XCTFail("unexpecting thrown error: \(error.localizedDescription)")
      fatalError()
    }
  ...
  private var expectationForManagerManagerFinished: XCTestExpectation?
  private var expectManagerManagerFinishedBlock: (PhotoUploadManager, Bool) throws -> Void = { _,_  in
    XCTFail("unexpected call to PhotoUploadManagerDelegate.manager(_:didFinishUploadingPhotos:)")
    fatalError()
  }

  @discardableResult
  func expectManagerManagerFinished(_ block: @escaping (PhotoUploadManager, Bool) throws -> Void) -> XCTestExpectation {
    self.expectationForManagerManagerFinished = self.testCase.expectation(description: "PhotoUploadManagerDelegate.manager(_:didFinishUploadingPhotos:)")
    self.expectManagerManagerFinishedBlock = block
    return self.expectationForManagerManagerFinished!
  }

  func manager(_ manager: PhotoUploadManager,
               didFinishUploadingPhotos finished: Bool  ) -> Void {
    XCTAssertNotNil(self.expectationForManagerManagerFinished)
    self.expectationForManagerManagerFinished?.fulfill()
    do {
      return try self.expectManagerManagerFinishedBlock(manager, finished)
